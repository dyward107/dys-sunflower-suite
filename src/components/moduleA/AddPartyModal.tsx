...
// Modal for adding parties to a case

import React, { useState } from 'react';
import { useCaseStore } from '../../stores/caseStore';
import type { PartyInput } from '../../types/ModuleA';
...
export const AddPartyModal: React.FC<AddPartyModalProps> = ({
  ...
  const { addCaseParty, isLoading } = useCaseStore();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">
          Add {partyType === 'defendant' ? 'Defendant' : 'Plaintiff'}
        </h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            >
              {isLoading ? 'Adding...' : `Add ${partyType === 'defendant' ? 'Defendant' : 'Plaintiff'}`}
            </button>
          </div>
        </form>
      </div>
